...
### sentinel 四种降级规则
1. RT：平均响应时间超过阈值(比如设置200ms)，并且每秒的请求数>=5,则断路器打开
   ![img18.png](img/img_18.png)
上图为RT规则图例
![img16.png](img/img_16.png)
上图为RT规则配置
2. 异常比例：异常比例超过秒级统计的阈值(取值[0.0 - 1.0]，即0%~100%)，并且每秒请求数>=5，则断路器打开
   ![img17.png](img/img_17.png)
 - 比如设置0.5，1秒内来了10次请求，其中5个请求异常，则异常比例=5/10=0.5，超过阈值，则断路器打开
3. 异常数：当资源1分钟内的异常数据超过阈值之后进行熔断，注意时间窗口一定要大于60s，否则结束熔断后仍可能再次进入熔断
   ![img19.png](img/img_19.png)
### 热点规则
- 按照资源的某个参数进行流控，相比上面的流控和降级规则，这个更精准，可以针对某个参数进行限流
![img20.png](img/img_20.png)
- 比如上图的配置，意思是当第一个参数的请求超过阈值(qps=1)时，请求会被限流，而单独的第二个参数的请求不受影响
